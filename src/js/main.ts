...
function updateLeftNavFriendsCount(count: number | string): void {
  const leftNavFriendsCount = document.querySelector<HTMLSpanElement>('a[href*="friends"] div.foundation-web-badge span');
    ...
    if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
      ...
      for (const node of mutation.addedNodes) {
        if (node.nodeType === 1) {
          ...
          if (element.matches('a[href*="friends"] div.foundation-web-badge span') || element.querySelector('a[href*="friends"] div.foundation-web-badge span')) {
